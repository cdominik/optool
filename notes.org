...
+ =-radmc3d [LABEL]= ::
  ...
  the scattering matrix[fn:2]. With this switch, the extension of the files
  will be changed to =.inp=, the normalization will be modified, and
  if you specify LABEL, it will be used in the file name(s).

[fn:2] The default normalization in =optool= is such that $F_{11}$
averages out to 1/steradian, i.e. $\oint F_{11} d\Omega = 4\pi$.  For
RADMC-3D, the normalization is $\oint Z_{11} d\Omega = \kappa_{\rm
scat}$.
