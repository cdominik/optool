subroutine manual(what)
  ...
  if (what.eq.'all') then
     ...
     write(*,'("	     ______________________________________________")')
     write(*,'("")')
     write(*,'("		       April 2022, version 1.9.6")')
     write(*,'("")')
  ...
  if(what.eq.'all') then
     ...
     write(*,'("  in the background[4] and puts all computed quantities as  numpy ")')
     ...
     write(*,'("   ----")')
     write(*,'("")')
     write(*,'("  The argument to  optool.particle()  must be a valid shell command[5]")')
     ...
     write(*,'("  - *microns* for grain sizes and wavelengths.[6]")')
     ...
     write(*,'("   normal distribution[7], triggered by sig<0      n(a) ~ exp[-0.5(\frac{a-a_m}{sig})^2]")')
     write(*,'("")')
     ...
     write(*,'("        specified by the *-a* switch is interpreted as the /compact/[8]")')
