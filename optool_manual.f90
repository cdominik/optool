subroutine manual(what)
  ...
  if (what.eq.'all') then
     ...
     write(*,'("             ______________________________________________")')
     write(*,'("")')
     write(*,'("                      November 2022, version 1.9.9")')
     write(*,'("")')
  ...
  if((what.eq.'-mie').or.(what.eq.'all')) then
     write(*,'("   -mie ")')
     write(*,'("        Do a standard Mie calculattion for perfect spheres.  This is")')
  ...
  if(what.eq.'all') then
     ...
     write(*,'("        wavelength (interval)[4] for the matrix to be stored. Multiple")')
  endif
  ...
  if(what.eq.'all') then
     ...
     write(*,'("    ,----")')
     write(*,'("    | sil = optool.particle( optool -d ol-mg50 -na 100 ,cache= sil )")')
     write(*,'("     ----")')
     ...
     write(*,'("         optool  switches from DHS to Mie[9]. In rare cases,  DMiLay ")')
     ...
     write(*,'("        specified by the *-a* switch is interpreted as the /compact/[10]")')
     ...
     write(*,'("==================")')
     write(*,'("")')
     write(*,'("  1. If you get oscillations in the opacities, in particular at long")')
     write(*,'("     wavelengths, the grain size resolution is not sufficient.  Use more")')
     write(*,'("     grain sizes (*-a*, *-na* and *-d* switches).")')
     write(*,'("  2. You may have noticed that  optool  became slower in version 1.9.9")')
