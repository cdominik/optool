subroutine manual(what)
  ...
  if (what.eq.'all') then
     ...
     write(*,'("	     ______________________________________________")')
     write(*,'("")')
     write(*,'("		       April 2022, version 1.9.5")')
     write(*,'("")')
  ...
  if((what.eq.'-a').or.(what.eq.'-amin').or.(what.eq.'-amax').or.(what.eq.'-apow').or.(what.eq.'-na').or.(what.eq.'all')&
     ...
     write(*,'("        Specify the mean size and the logarithmic width for a [log")')
     write(*,'("        normal size distribution].  You may also use *-amean* and")')
     write(*,'("        *-asig* options to set these values.")')
     write(*,'("")')
  endif
  if(what.eq.'all') then
  ...
  if(what.eq.'all') then
     ...
     write(*,'("  sigma=1.2:")')
     write(*,'("  ,----")')
     write(*,'("  | sil_ln = sil.sizedist( np.exp( (np.log(sil.a1/1.3)/1.2)**2) )")')
     ...
     write(*,'("         optool  can use powerlaw and log-normal size distributions out")')
     write(*,'("        of the box.  Each of these will be subject to a minimum and a")')
     write(*,'("        maximum grain size. The grain size grid is logarithmic, so")')
     write(*,'("        $da\propto a$. The powerlaw fills logarithmic size bins")')
     write(*,'("        according to $n(a)\propto a^{-p+1}$.  The log-normal")')
     write(*,'("        distribution uses $n(a)\propto \exp\left[-(\log_{10} (a/a_{\rm")')
     write(*,'("        m})\,/\sigma)^2\right]$.  Other size distributions can be")')
     write(*,'("        constructed using the [python interface].")')
     write(*,'("")')
