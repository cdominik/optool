subroutine manual()
  ...
  write(*,'("        [scattering matrix]. File names will contain  LABEL  if")')
  write(*,'("        specified and have the extension  .inp .")')
  write(*,'("")')
  write(*,'("   -print [VAR] ")')
  write(*,'("        Write to  STDOUT  instead of files. The default is to write")')
  write(*,'("        lambda, k_abs, kappa_scat, kappa_ext, and g. When  VAR  is")')
  write(*,'("         kabs ,  ksca ,  kext , or  g , write only that quantity.  You")')
  write(*,'("        can use this to extract a single value, for example the 850mum")')
  write(*,'("        extinction opacity of grains between 1 and 3mm:  optool -a 1000")')
  write(*,'("        3000 -l 850 -print kext ")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  ...
  write(*,'("")')
  write(*,'("  -c Key      -c Key       Material                 State        _max   Referen")')
  write(*,'(" generic     full key                                          g/ mum          ")')
  write(*,'("-------------------------------------------------------------------------------")')
  write(*,'("             pyr-mg100    MgSiO_3                  amorph         500  [Dorschn")')
  write(*,'("             pyr-mg95     Mg_0.95  Fe_0.05  SiO_3  amorph         500  [Dorschn")')
  write(*,'("             pyr-mg80     Mg_0.8  Fe_0.2  SiO_3    amorph         500  [Dorschn")')
  write(*,'(" \fboxpyr    pyr-mg70     Mg_0.7  Fe_0.3  SiO_3    amorph         500  [Dorschn")')
  write(*,'("             pyr-mg60     Mg_0.6  Fe_0.4  SiO_3    amorph         500  [Dorschn")')
  write(*,'("             pyr-mg50     Mg_0.5  Fe_0.5  SiO_3    amorph         500  [Dorschn")')
  write(*,'("             pyr-mg40     Mg_0.4  Fe_0.6  SiO_3    amorph         500  [Dorschn")')
  write(*,'(" ens         pyr-c-mg96   Mg_0.96  Fe_0.04  SiO3   cryst          99   [Jäger+")')
  write(*,'("-------------------------------------------------------------------------------")')
  write(*,'(" ol          ol-mg50      MgFeSiO_4                amorph         500  [Dorschn")')
  write(*,'("             ol-mg40      Mg_0.8  Fe_1.2  SiO_4    amorph         500  [Dorschn")')
  write(*,'(" for         ol-c-mg100   Mg_2 SiO_4               cryst          250  [Steyer+")')
  write(*,'("-------------------------------------------------------------------------------")')
  write(*,'("             astrosil     MgFeSiO_4                mixed          1e5  [Draine+")')
  write(*,'("-------------------------------------------------------------------------------")')
  write(*,'(" \fboxc      c-z          C                        amorph?        1e4  [Zubko+9")')
  write(*,'("             c-p          C                        amorph         800  [Preibis")')
  write(*,'(" gra         c-gra        C graphite               cryst        21000  [Draine+")')
  write(*,'(" org         c-org        CHON organics            amorph         1e5  [Henning")')
  write(*,'("             c-nano       C nano-diamond           cryst         110   [Mutschk")')
  write(*,'("-------------------------------------------------------------------------------")')
  write(*,'(" iron        fe-c         Fe                       metal          1e5  [Henning")')
  write(*,'("             fes          FeS                      metal          1e5  [Henning")')
  write(*,'("             sic          SiC                      cryst         1000  [Laor93]")')
  write(*,'("-------------------------------------------------------------------------------")')
  write(*,'(" qua         sio2         SiO_2                    amorph         500  [Kitamur")')
  write(*,'(" cor         cor-c        Al_2  O_3                cryst          40   [Koike+9")')
  write(*,'("-------------------------------------------------------------------------------")')
  write(*,'(" \fboxh2o    h2o-w        Water ice                cryst          2e6  [Warren+")')
  write(*,'("             h2o-a        Water ice                amorph         2e6  [Hudgins")')
  write(*,'(" co2         co2-w        CO_2 ice                 cryst          2e5  [Warren+")')
  write(*,'(" nh3         nh3-m        NH_3 ice                 cryst          200  [Martonc")')
  write(*,'(" co          co-a         CO ice                   amorph        5.8   [Palumbo")')
  write(*,'("             co2-a / c    CO_2 ice                 am / cr        20   [Gerakin")')
  write(*,'("             ch4-a / c    CH_4 ice                 am / cr        20   [Gerakin")')
  write(*,'("             ch3oh-a / c  CH_3  OH ice             am / cr  0.78/ 24   [Gerakin")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  ...
  write(*,'("        gyration $R_{\rm g}$ of the aggregate by")')
  write(*,'("        \begin{equation}")')
  write(*,'("         N=k_{\rm f}\left(\frac{R_{\rm g}}{a_0}\right)^{D_{\rm f}}")')
  write(*,'("        \end{equation}")')
  write(*,'("        The size $a$ of the particles as specified by the *-a* switch is")')
  write(*,'("        interpreted as the /compact/[6] size of all material in the")')
  ...
  write(*,'("        by $f=N\cdot\left(\sqrt{3/5}\,a_0/R_{\rm g}\right)^3$.  To")')
  write(*,'("        determine the structure of the aggregates, the user can specify")')
  write(*,'("        a structure parameter.  If that parameter is larger than 1, it")')
  ...
  write(*,'("        will decrease with aggregate size.  As an alternative, the")')
  write(*,'("        structure parameter can be less than 1.  In that case, it is")')
  ...
  write(*,'("        prefactor $k_{\rm f}$ is chosen automatically so that the")')
  write(*,'("        asymptotic density of small aggregates is the monomer material")')
  write(*,'("        density. To force another value for the prefactor, it can be")')
  write(*,'("        given explicitly as the third value of the  -mmf  option. The")')
  write(*,'("        following table summarizes the relevant equations.")')
  write(*,'("")')
  write(*,'("                   |  -mmf A0 DF           |  -mmf A0 FILL                     ")')
  write(*,'("      -------------+-----------------------+-----------------------------------")')
  write(*,'("       $f$         | $N^(D_\rm f  -3)/3  $ |  given by user      rm f  ^3/D_\r")')
  write(*,'("       $D_\rm f  $ |  given by user        | $3\ln N\,/\,\ln(N/f)              ")')
  write(*,'("       $k_\rm f  $ | $(5/3)^D_\rm f  /2  $ | $(5/3)^D_\rm f  /2                ")')
  write(*,'("")')
  ...
  write(*,'("    the file header - it contains the exact command that was used to")')
