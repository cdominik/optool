subroutine manual(what)
  ...
  if((what.eq.'-w').or.(what.eq.'all')) then
     ...
     write(*,'("        grain size distribution and the wavelengths grid,")')
     write(*,'("        respectively. Also, write  blended.lnk  with the result of")')
  ...
  if(what.eq.'all') then
     ...
     write(*,'(" ens         pyr-c-mg96   Mg_0.96  Fe_0.04  SiO3   cryst[3 [Jäger+98")')
     write(*,'("---------------------------------------------------------- --------------")')
     write(*,'(" ol          ol-mg50      MgFeSiO_4                amorph  [Dorschner+95")')
     write(*,'("             ol-mg40      Mg_0.8  Fe_1.2  SiO_4    amorph  [Dorschner+95")')
     write(*,'(" for         ol-c-mg100   Mg_2  SiO_4              cryst[3 [Suto+06")')
     write(*,'("             ol-c-mg95    Mg_1.9  Fe_0.1  SiO_4    cryst[3 [Fabian+01")')
     write(*,'(" fay         ol-c-mg00    Fe_2  SiO_4              cryst[3 [Fabian+01")')
     write(*,'("---------------------------------------------------------- --------------")')
     write(*,'("             astrosil     MgFeSiO_4                mixed   [Draine+03")')
     write(*,'("---------------------------------------------------------- --------------")')
     write(*,'("      c      c-z          C                        amorph? [Zubko+96")')
     write(*,'("             c-p          C                        amorph  [Preibisch+93")')
     write(*,'(" gra         c-gra        C graphite               cryst[3 [Draine+03")')
     ...
     write(*,'("        for it.  Also, see the  -radmc  switch which will modify[5] the")')
     ...
     write(*,'("        wavelength (interval)[6] for the matrix to be stored. Multiple")')
  endif
  if((what.eq.'-sp').or.(what.eq.'all')) then
     ...
     write(*,'("")')
     write(*,'("  blended.lnk")')
     write(*,'("         optool  mixes the optical properties of the various materials")')
     write(*,'("        specified to be part of the grains into a single set of optical")')
     write(*,'("        properties that are then used in the scattering")')
     write(*,'("        computations. When the  -b  or  -w  switch is present,  optool ")')
     write(*,'("        will write the result of the mixing process into the file")')
     write(*,'("         blended.lnk .")')
     write(*,'("")')
     write(*,'("  optool_sd.dat & optool_lam.dat")')
     write(*,'("        When running  optool  with the  -w  switch, it will write the")')
     write(*,'("        grain size grid and the wavelength grid to two files.  You can")')
     write(*,'("        use this to verify that the grids set up are what you expected.")')
     write(*,'("        The files are also example files for what the structure of files")')
     write(*,'("        need to be to be read in with  -a file.dat  and  -l file.dat ")')
     write(*,'("        for user-provided size and wavelength grids.")')
     write(*,'("")')
  endif
  if(what.eq.'all') then
     ...
     write(*,'("  in the background[7] and puts all computed quantities as  numpy ")')
     ...
     write(*,'("   ----")')
     write(*,'("")')
     write(*,'("  The argument to  optool.particle()  must be a valid shell command[8]")')
     ...
     write(*,'("    module and [appendix D.1] for examples.")')
     write(*,'("")')
     write(*,'("8 Units")')
     write(*,'("=======")')
     write(*,'("")')
     ...
     write(*,'("  - *microns* for grain sizes and wavelengths.[9]")')
     ...
     write(*,'("  4. If after successful compilation the command  optool  does not work")')
     write(*,'("     in your environment, make sure  optool  is on the binary search")')
     write(*,'("     path of your shell, or run it by giving the full path to the")')
     write(*,'("     executable, like  ./optool .")')
