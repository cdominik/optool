subroutine manual()
  ...
  write(*,'("")')
  write(*,'("			   OP-TOOL USER GUIDE")')
  write(*,'("")')
  write(*,'("			       C. Dominik")')
  write(*,'("			  ____________________")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("			July 2020, version 0.9.7")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  ...
  write(*,'("============")')
  write(*,'("")')
  write(*,'("  This tool allows to produce complex dust opacities right from the")')
  write(*,'("  command line. It is derived from Michiel Min s [OpacityTool] for the")')
  write(*,'("  [DIANA] program, and committed to its simplicity. For another")')
  write(*,'("  derivative of OpacityTool with a different set of applications, check")')
  write(*,'("  out [SIGMA], the /Simple Icy Grain Model for Aggregates/, by Lefévre")')
  write(*,'("  et al. (2020).")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("Capabilities")')
  ...
  write(*,'("~~~~~~~~~~~~")')
  write(*,'("")')
  write(*,'("  The code doing the real work of  optool  was adopted and modified from")')
  write(*,'("  [OpacityTool]. I focused on the interface, on simplicity, and a clean")')
  write(*,'("  set of conventions. If you are using this tool, you should point to")')
  write(*,'("  the [GitHub repository] and give credit to the papers describing the")')
  write(*,'("  methods used to compute the opacities, as well as to the original")')
  ...
  write(*,'("  - DIANA standard Opacities: [Woitke, Min et al. 2016]")')
  ...
  write(*,'("  cores[1] for speed, or to be able to write FITS files[2], use one or")')
  ...
  write(*,'("========")')
  write(*,'("")')
  write(*,'("  A simple grain made only of the default pyroxene")')
  write(*,'("  ,----")')
  write(*,'("  | optool -c pyr")')
  write(*,'("   ----")')
  write(*,'("")')
  write(*,'("  Reproduce the DIANA standard dust model, using the pyroxene with 70%")')
  write(*,'("  magnesium, and 13% (by mass) carbon, and 25% porosity.")')
  write(*,'("  ,----")')
  write(*,'("  | optool -c pyr-mg70 0.87  -c c-z 0.13  -p 0.25")')
  write(*,'("   ----")')
  write(*,'("")')
  ...
  write(*,'("   ----")')
  write(*,'("")')
  write(*,'("  Add an ice mantle that is 20% of the core mass")')
  write(*,'("  ,----")')
  write(*,'("  | optool -c pyr-mg70 0.87  -c c-z 0.13  -m ice-w 0.2 -p 0.25")')
  write(*,'("   ----")')
  write(*,'("")')
  ...
  write(*,'("")')
  write(*,'("  ,----")')
  write(*,'("  | optool -c pyr-mg70 0.87  -c c-z 0.13  -m ice_hudgins.dat 0.2  -p 0.25")')
  write(*,'("   ----")')
  write(*,'("")')
  ...
  write(*,'("  distribution. Also, restrict the wavelength range to 10-100mum.")')
  write(*,'("")')
  write(*,'("  ,----")')
  write(*,'("  | optool -c ice-w 1.0 -a 1 3 2.5 15 -l 10 100")')
  write(*,'("   ----")')
  write(*,'("")')
  write(*,'("  Represent the default dust model (DIANA) in 42 grain sizes, and")')
  write(*,'("  produce input files for RADMC-3D, one for each grain size, with full")')
  write(*,'("  scattering matrix, chopping 3 degrees from the scattering peak.")')
  write(*,'("")')
  write(*,'("  ,----")')
  ...
  write(*,'("          Specify a material to include into the grain.   KEYorFILE  can")')
  write(*,'("        be the [key for a builtin material], or the path to the correct")')
  write(*,'("         lnk  file.  MFRAC  is the /mass/ fraction of the material. You")')
  write(*,'("        can give up to 10 *-c* switches to add the different")')
  write(*,'("        contributions.  All materials will be mixed together using the")')
  write(*,'("        Bruggeman rule, and vacuum can be included through the porosity.")')
  write(*,'("")')
  write(*,'("   -m KEY-or-FILE [MFRAC] ")')
  write(*,'("          Like *-c*, but use this material as a mantle material that")')
  write(*,'("        will be mixed in using the Maxwell-Garnett rule instead of the")')
  write(*,'("        Bruggeman rule.  Only one *-m* switch is allowed, and it needs")')
  write(*,'("        to come after all *-c* specifications.")')
  write(*,'("")')
  ...
  write(*,'("          Specify minimum and maximum grain radius, and optionally the")')
  write(*,'("        size distribution powerlaw and the number of size bins.  You can")')
  write(*,'("        also use options to set individual values with *-amin*, *-amax*,")')
  write(*,'("        *-apow*, *-na*. The defaults are 0.05 mum, 3000 mum, 3.5, and")')
  write(*,'("        /10 per size decade with a fixed minimum of 5/, respectively.")')
  write(*,'("")')
  write(*,'("")')
  ...
  write(*,'("          Minimum and maximum wavelength and optionally the number of")')
  write(*,'("        wavelengths points for the construction of the wavelength grid.")')
  write(*,'("        The default values are 0.05 mum, 10000 mum, and 300,")')
  write(*,'("        respectively.  Use the options *-lmin*, *-lmax*, and *-nlam* (or")')
  write(*,'("        *-nl*) to set individual values.")')
  write(*,'("")')
  write(*,'("   -l FILE ")')
  write(*,'("          Read the wavelength grid from the first column of  FILE .  The")')
  write(*,'("        first number in the file must be the number of grid points. So")')
  write(*,'("        this could be an [ lnk ] file.")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  ...
  write(*,'("")')
  write(*,'("   *-c Key*  *-c Key*    *Material*               *State*       rho  lambda_min  lambda_max  *Reference*     ")')
  write(*,'("   generic   full key                                      [g/cm^3]       [mum]       [mum]                  ")')
  write(*,'("  -----------------------------------------------------------------------------------------------------------")')
  write(*,'("             pyr-mg100   MgSiO_3                  amorph       2.71         0.2         500  [Dorschner+95]  ")')
  write(*,'("             pyr-mg95    Mg_{0.95}Fe_{0.05}SiO_3  amorph       2.74         0.2         500  [Dorschner+95]  ")')
  write(*,'("             pyr-mg80    Mg_{0.8}Fe_{0.2}SiO_3    amorph        2.9         0.2         500  [Dorschner+95]  ")')
  write(*,'("   pyr       pyr-mg70    Mg_{0.7}Fe_{0.3}SiO_3    amorph       3.01         0.2         500  [Dorschner+95]  ")')
  write(*,'("             pyr-mg60    Mg_{0.6}Fe_{0.4}SiO_3    amorph        3.1         0.2         500  [Dorschner+95]  ")')
  write(*,'("             pyr-mg50    Mg_{0.5}Fe_{0.5}SiO_3    amorph        3.2         0.2         500  [Dorschner+95]  ")')
  write(*,'("             pyr-mg40    Mg_{0.4}Fe_{0.6}SiO_3    amorph        3.3         0.2         500  [Dorschner+95]  ")')
  write(*,'("   ens       pyr-c-mg96  Mg_{0.96}Fe_{0.04}SiO3   cryst         2.8       *2.0*        *99*  [Jäger+98]     ")')
  write(*,'("  -----------------------------------------------------------------------------------------------------------")')
  write(*,'("   ol        ol-mg50     MgFeSiO_4                amorph       3.71         0.2         500  [Dorschner+95]  ")')
  write(*,'("             ol-mg40     Mg_0.8 Fe_1.2 SiO_4      amorph       3.71         0.2         500  [Dorschner+95]  ")')
  write(*,'("   for       ol-c-mg100  Mg_2 SiO_4               cryst        3.33       *3.0*         250  [Steyer+74]     ")')
  write(*,'("  -----------------------------------------------------------------------------------------------------------")')
  write(*,'("   c         c-z         C                        amorph?       1.8        0.05         1e4  [Zubko+96]      ")')
  write(*,'("             c-p         C                        amorph        1.8        0.11         800  [Preibisch+93]  ")')
  write(*,'("   gra       c-gra       C graphite               cryst       2.16?       0.001        1000  XXX+1900        ")')
  write(*,'("             c-org       CHON organics            amorph        1.4         0.1         1e5  Henning+1900    ")')
  write(*,'("             c-nano      C nano-diamond           cryst         2.3        0.02       *110*  [Mutschke+04]   ")')
  write(*,'("  -----------------------------------------------------------------------------------------------------------")')
  write(*,'("   ice       ice-w       Water ice                cryst        0.92        0.04         2e6  [Warren+08]     ")')
  write(*,'("  -----------------------------------------------------------------------------------------------------------")')
  write(*,'("   iron      fe-c        Fe                       metal        7.87         0.1         1e5  [Henning+96]    ")')
  write(*,'("             fes         FeS                      metal        4.83         0.1         1e5  [Henning+96]    ")')
  write(*,'("             sic         SiC                      cryst        3.22       0.001        1000  [Draine93]      ")')
  write(*,'("  -----------------------------------------------------------------------------------------------------------")')
  write(*,'("   cor       cor-c       Al_{2}O_3                cryst         4.0         0.5        *40*  [Koike+95]      ")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  ...
  write(*,'("  have an internal way to deal with this. An example would be [MCMax3D],")')
  write(*,'("  which has the  nspike  keyword for exactly this purpose.  Others, like")')
  write(*,'("  RADMC-3D require this to be taken care of by the process that creates")')
  write(*,'("  the opacity files.  This is the purpose of the  -chop  switch in")')
  ...
  write(*,'("  scaled.  As a result, in effect, the radiation that would be")')
  write(*,'("  /scattered/ into this narrow range of angles will be treated as if it")')
  write(*,'("  did have /no interaction at all/ with the grain.")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("Angular grid")')
  ...
  write(*,'("  So for for the input files for RADMC-3D, we interpolate and extend the")')
  write(*,'("  computed values to the cell boundaries.  In strong forward-scattering")')
  write(*,'("  grains, this does require a small renormalization to make sure the the")')
  write(*,'("  full solid angle integration gives again accurate results.  optool  is")')
  write(*,'("  handling this renormalization fully automatically.")')
  write(*,'("")')
  write(*,'("")')
