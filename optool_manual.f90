subroutine manual(what)
  ...
  if (what.eq.'all') then
     ...
     write(*,'("             ______________________________________________")')
     write(*,'("")')
     write(*,'("                      October 2022, version 1.9.8")')
     write(*,'("")')
  ...
  if((what.eq.'-diana').or.(what.eq.'all')) then
     ...
     write(*,'("        Use literature standard compositions DIANA (Woitke+2016) or")')
     write(*,'("        DSHARP (Birnstiel+2018).")')
     write(*,'("")')
  endif
  if(what.eq.'all') then
  ...
  if((what.eq.'-s').or.(what.eq.'all')) then
     ...
     write(*,'("        change the the number of equally-spaced [angular grid points] to")')
  ...
  if(what.eq.'all') then
     ...
     write(*,'("        for it.  Also, see the  -radmc  switch which will modify[3] the")')
     write(*,'("        output to make sure it can be used as an input file for")')
     write(*,'("        [RADMC-3D].")')
     write(*,'("")')
     write(*,'("        To save space,  optool  can write a /sparse file/ that stores")')
     write(*,'("        the full scattering matrix only for selected wavelengths (for")')
     write(*,'("        example, the ones that will be used for image generation). Use")')
  endif
  if((what.eq.'-sparse').or.(what.eq.'all')) then
     write(*,'("         -sparse LAM  or  -sparse LAM1 LAM2  to define a wavelength")')
     write(*,'("        (interval)[4] for the matrix to be stored. Multiple  -sparse ")')
     write(*,'("        switches are allowed.")')
     write(*,'("")')
  ...
  if(what.eq.'all') then
     ...
     write(*,'("  in the background[5] and puts all computed quantities as  numpy ")')
     ...
     write(*,'("   ----")')
     write(*,'("")')
     write(*,'("  The argument to  optool.particle()  must be a valid shell command[6]")')
     ...
     write(*,'("  sigma=1.2:")')
     write(*,'("  ,----")')
     write(*,'("  | sil_ln = sil.sizedist( np.exp( -0.5*(np.log(sil.a1/1.3)/1.2)**2) )")')
     ...
     write(*,'("  - *microns* for grain sizes and wavelengths.[7]")')
     ...
     write(*,'("   normal distribution[8], triggered by sig<0      n(a) ~ exp[-0.5(\frac{a-a_m}{sig})^2]")')
     write(*,'("")')
     ...
     write(*,'("")')
     write(*,'("12 Overview of optical properties")')
     write(*,'("=================================")')
     write(*,'("")')
     ...
     write(*,'("        Numerical inaccuracies in the  DMiLay  routine can lead to an")')
     write(*,'("        unphysical result with $q_{\rm sca}>q_{\rm ext}$, implying a")')
     write(*,'("        negative $q_{\rm abs}$, in a situation where the imaginary part")')
     write(*,'("        of the refractive index is extremely small.   optool  enforces")')
     write(*,'("        $q_{\rm abs}>q_{\rm ext}/10^4$, with no consequences for")')
     write(*,'("        practical applications.")')
     write(*,'("")')
     write(*,'("  MMF")')
     ...
     write(*,'("        specified by the *-a* switch is interpreted as the /compact/[9]")')
