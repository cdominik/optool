subroutine manual(what)
  ...
  if (what.eq.'all') then
     ...
     write(*,'("             ______________________________________________")')
     write(*,'("")')
     write(*,'("                       April 2023, version 1.9.11")')
     write(*,'("")')
     ...
     write(*,'("  and also  optool2tex  and  optool-complete  into  bindir .")')
     write(*,'("")')
     write(*,'("  For shell command line completion support, check the file")')
  ...
  if((what.eq.'-a').or.(what.eq.'-amin').or.(what.eq.'-amax').or.(what.eq.'-apow').or.(what.eq.'-na').or.(what.eq.'all')&
     ...
     write(*,'("        described in [appendix B].  To get an example file")')
     write(*,'("         optool_sd.dat , run  optool  with the option *-wgrid*.")')
     write(*,'("")')
  endif
  if(what.eq.'all') then
  ...
  if((what.eq.'-l').or.(what.eq.'-lmin').or.(what.eq.'-lmax').or.(what.eq.'-nl').or.(what.eq.'-nlam').or.(what.eq.'all')) then
     ...
     write(*,'("         optool_lam.dat , run  optool  with the option *-wgrid*. An")')
     write(*,'("        [ lnk ] file could be used here as well!")')
     write(*,'("")')
  endif
  if(what.eq.'all') then
  ...
  if((what.eq.'-wgrid').or.(what.eq.'all')) then
     write(*,'("   -wgrid ")')
     write(*,'("        Write the files  optool_sd.dat  and  optool_lam.dat  with the")')
     write(*,'("        grain size distribution and the wavelengths grid, respectively.")')
     write(*,'("")')
  endif
  if(what.eq.'all') then
     ...
     write(*,'(" for         ol-c-mg100   Mg_2  SiO_4              cryst[2 [Suto+06")')
     write(*,'("             ol-c-mg95    Mg_1.9  Fe_0.1  SiO_4    cryst[2 [Fabian+01")')
     write(*,'(" fay         ol-c-mg00    Fe_2  SiO_4              cryst[2 [Fabian+01")')
     write(*,'("---------------------------------------------------------- --------------")')
     write(*,'("             astrosil     MgFeSiO_4                mixed   [Draine+03")')
     write(*,'("---------------------------------------------------------- --------------")')
     write(*,'("      c      c-z          C                        amorph? [Zubko+96")')
     write(*,'("             c-p          C                        amorph  [Preibisch+93")')
     write(*,'(" gra         c-gra        C graphite               cryst[2 [Draine+03")')
     ...
     write(*,'("        for it.  Also, see the  -radmc  switch which will modify[4] the")')
     ...
     write(*,'("        wavelength (interval)[5] for the matrix to be stored. Multiple")')
  endif
  ...
  if(what.eq.'all') then
     ...
     write(*,'("  in the background[6] and puts all computed quantities as  numpy ")')
     ...
     write(*,'("   ----")')
     write(*,'("")')
     write(*,'("  The argument to  optool.particle()  must be a valid shell command[7]")')
     ...
     write(*,'("    module and [appendix D.2] for examples.")')
     write(*,'("")')
     write(*,'("8 Units")')
     write(*,'("=======")')
     write(*,'("")')
     ...
     write(*,'("  - *microns* for grain sizes and wavelengths.[8]")')
     ...
     write(*,'("   normal distribution[9], triggered by sig<0      n(a) ~ exp[-0.5(\frac{a-a_m}{sig})^2]")')
     write(*,'("")')
     ...
     write(*,'("  run  optool  with the option *-wgrid*):")')
     write(*,'("")')
     ...
     write(*,'("=============================")')
     write(*,'("")')
     write(*,'("11.1 How to ingest refractive index data for another material")')
     write(*,'("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")')
     write(*,'("")')
     ...
     write(*,'("")')
     write(*,'("11.2 Crystalline materials")')
     write(*,'("~~~~~~~~~~~~~~~~~~~~~~~~~~")')
     write(*,'("")')
     ...
     write(*,'("  materials built into optool we assumed that the material consists of")')
     write(*,'("  many small crystalline areas that are randomly oriented within each")')
     write(*,'("  grain. For this, the refractive index data for different orientations")')
     write(*,'("  have been combined using the Bruggeman effective medium rule. It")')
     write(*,'("  results in a single refractive index data set that is easy to")')
     write(*,'("  use. However, we can also think of a situation where each dust grain")')
     write(*,'("  is a single crystal in a cloud of randomly oriented grains. In that")')
     write(*,'("  case, we need to compute opacities for individual orientations, and")')
     write(*,'("  then average the opacities. Where available, the axis-specific data is")')
     write(*,'("  also included in the optool distribution, in the  lnk_data/cad ")')
     write(*,'("  directory. You could use the python interface to do this kind of")')
     write(*,'("  mixing in the following way:")')
     write(*,'("  ,----")')
     write(*,'("  | import optool")')
     write(*,'("  | px = optool.particle( optool -a 1.5 lnk_data/cad/c-gra-x-Draine2003.lnk )")')
     write(*,'("  | py = optool.particle( optool -a 1.5 lnk_data/cad/c-gra-y-Draine2003.lnk )")')
     write(*,'("  | pz = optool.particle( optool -a 1.5 lnk_data/cad/c-gra-z-Draine2003.lnk )")')
     ...
     write(*,'("         optool  switches from DHS to Mie[10]. In rare cases,  DMiLay ")')
     ...
     write(*,'("        specified by the *-a* switch is interpreted as the /compact/[11]")')
     ...
     write(*,'("  3. You may have noticed that  optool  became slower in version 1.9.9")')
     ...
     write(*,'("     grains. However, we wanted to make sure the user always gets the")')
     write(*,'("     best possible results out of the box, which is why we increased the")')
     write(*,'("     limit.")')
     write(*,'("  4. If you do not remember how to reproduce a specific run, just check")')
     ...
     write(*,'("     optool2tex .")')
     write(*,'("  - Thiébaut Schirmer for triggering the addition of a log-normal size")')
     write(*,'("    distribution.")')
     write(*,'("")')
