subroutine manual()
  ...
  write(*,'("")')
  write(*,'("")')
  write(*,'("		       December 2020, version 1.5")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("")')
  ...
  write(*,'("        (Toon & Ackerman 1981).  However, when the size parameter $x=2")')
  write(*,'("        \pi a/\lambda$ exceeds a value of 10^4, then no DHS averaging is")')
  write(*,'("        used.  A standard Mie calculation is performed , using the")')
  write(*,'("        routine  MeerhoffMie  (reference missing), for a fixed size")')
  write(*,'("        parameter of 5000, with proper scaling to the actual size of the")')
  write(*,'("        particle.")')
  write(*,'("")')
  write(*,'("  MMF")')
  ...
  write(*,'("        \end{equation}")')
  write(*,'("         The size $a$ of the particles as specified by the *-a* switch")')
  write(*,'("        is interpreted as the /compact/ size of all material in the")')
  write(*,'("        aggregate, so that simply $N=a^3/a_0^3$, where $a_0$ is the")')
  write(*,'("        radius of the monomer.  The average volume filling factor $f$")')
  write(*,'("        can be expressed by")')
  write(*,'("        $f=N\cdot\left(\sqrt{3/5}\,a_0/R_g\right)^3$.  To determine the")')
  write(*,'("        structure of the aggregates, the user can specify a structure")')
  write(*,'("        parameter.  If that parameter is larger than 1, it is")')
  write(*,'("        interpreted as the /fractal dimension/ $D_f$ of the aggregates.")')
  write(*,'("        Using a fixed fractal dimension means that the volume filling")')
  write(*,'("        factor will decrease with aggregate size.  As an alternative,")')
  write(*,'("        the structure parameter can be less than 1.  In that case, it is")')
  write(*,'("        interpreted as a fixed /volume filling factor/ $f$ that applies")')
  write(*,'("        to all aggregate sizes - with the implication that then the")')
  write(*,'("        fractal dimension increases as a function of size. The fractal")')
  write(*,'("        prefactor $k_f$ is chosen automatically so that the asymptotic")')
  write(*,'("        density of small aggregates is the monomer material density.  To")')
  write(*,'("        force another value for the prefactor, it can be given")')
  write(*,'("        explicitly as the third value of the  -mmf  option. The")')
  write(*,'("        following table summarises the relevant equations.")')
  write(*,'("")')
  ...
  write(*,'("        scattering matrix.  It also computes the phase shift")')
  write(*,'("        $\Delta\phi$ caused by the aggregate to check the validity of")')
  write(*,'("        the scattering matrix and the asymmetry parameter.  If the")')
  write(*,'("        condition $\Delta\phi<1$ for accurate scattering matrix results")')
  write(*,'("        is violated, a warning will be issued. However, even then, the")')
  write(*,'("        opacities will be applicable.")')
  write(*,'("")')
  write(*,'("")')
