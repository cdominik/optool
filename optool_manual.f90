subroutine manual()
  ...
  write(*,'("  new session, because of the  read=True  flag, if the directories still")')
  write(*,'("  exist, the same commands just read the computed opacities back in -")')
  write(*,'("  this is much faster than recomputing them every time.  Be careful,")')
  write(*,'("  however, when you change the command. Then you need to remove the")')
  write(*,'("  directories to ensure new opacities will be computed.")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("  ,----")')
  write(*,'("  | import optool")')
  write(*,'("  | import numpy as np")')
  write(*,'("  | sil  = optool.particle( optool -d -a 0.001 100 0 100 ol-mg50 , sil ,read=True)")')
  write(*,'("  | carb = optool.particle( optool -d -a 0.001 3.0 0 50  c , carb ,read=True)")')
  write(*,'("   ----")')
  write(*,'("")')
  write(*,'("")')
  ...
  write(*,'("    grain size resolution is not sufficient.  Use more grain sizes.")')
  write(*,'("")')
  write(*,'("")')
