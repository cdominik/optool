subroutine manual()
  ...
  write(*,'("")')
  write(*,'("  - a  lnktable  class to read, plot and modify  lnk  files.")')
  write(*,'("    ,----")')
  ...
  write(*,'("    of examples.")')
  write(*,'("  Pre-compute the opacities of 100 olivine silicate grain sizes and of")')
  write(*,'("  50 carbon grain sizes, and store the opacities in directories. This")')
  write(*,'("  works simply by specifying the directory as the second argument. In a")')
  write(*,'("  new session, because of the  cache=True  flag, if the directories")')
  write(*,'("  still exist, the same commands just read the computed opacities back")')
  write(*,'("  in - this is much faster than recomputing them every time.  Be")')
  write(*,'("  careful, however, when you change the command. Then you need to remove")')
  write(*,'("  the directories to ensure new opacities will be computed.")')
  write(*,'("")')
  write(*,'("")')
  write(*,'("  ,----")')
  write(*,'("  | import optool")')
  write(*,'("  | import numpy as np")')
  write(*,'("  | sil  = optool.particle( optool -d -a 0.001 100 0 100 ol-mg50 , sil ,cache=True)")')
  write(*,'("  | carb = optool.particle( optool -d -a 0.001 3.0 0 50  c , carb ,cache=True)")')
  write(*,'("   ----")')
  write(*,'("")')
  ...
  write(*,'("  a^{-3.5} on a logarithmic grid implies using a power a^{-2.5}.")')
  write(*,'("")')
  write(*,'("  ,----")')
  ...
  write(*,'("  ,----")')
  write(*,'("  | p_ism = 0.01*ptot                 # dilute the opacity")')
  ...
  write(*,'("==================")')
  write(*,'("")')
  write(*,'("  - If you get oscillations in the opacities at long wavelengths, the")')
  write(*,'("    grain size resolution is not sufficient.  Use more grain sizes (see")')
  write(*,'("    the *-a*, *-na* and *-d* switches).")')
  write(*,'("")')
  write(*,'("")')
