...
+ =-mmf [AMONO [DFRAC]]= ::
  ...
  then be aggregates with a /compact size/ given by *-a* switch,
  effectively specifying a mass.  The volume filling factor $f=1-p$ is
  specified through the /porosity/ (*-p*), and a fractal dimension
  will be chosen appropriately. a_mono is the monomer radius, with
  default 0.1\mu{}m.  Monomers will have the composition given by the
  *-c* and *-m* switches. D_frac is the fractal dimension - the
  default is to keep it variable to match the porosity given by the
  *-p* switch.  When D_frac is given, the porosity will be ignored.
...
3. In high-porosity aggregates, the properties of the monomers become
   ...
   where this is true, a warning will be issued.  The scattering
   matrix and $g$ factor should then NOT be used.
...
- MMF :: To apply MMF, =optool= first turns the information about the
  ...
  mono}^3$, by assuming that the size $a$ sets the /compact volume/ of
  the aggregate, ignoring porosity.  It then determines fractal
  dimension $D_f$, and a fractal prefactor $k_f$, which together set
  the structure of the aggregate.  If the user specifies a fractal
  dimention with the =-mmf= option, it is enforced throughout.  If no
  fractal dimension has been specified, the porosity $p$ is used to
  determine a filling factor $f=1-p$ for the aggregate, and $D_f$ is
  chosen (different for every size) to satisfy this condition. The
  fractal prefactor $k_f$ is computed from the asymptotal behavior
  that the filling factor of an aggregate of the size of a monomer
  should be unity.
  ...
  check the validity of the scattering matrix.
