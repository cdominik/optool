...
+ =-mmf [AMONO [DFRAC]]= ::
  ...
  have the composition given by the *-c* and *-m* switches, and fixed
  radii a_mono with default 0.1\mu{}m.  Particles will be aggregates
  with a /compact size/ given by the *-a* switch, so that the number
  of monomers is $N_{\rm mono}=a^3/a_{\rm mono}^3$. The /porosity/
  (*-p*) specifies how the monomers should be spread out to reach the
  corresponding filling factor $f=1-p$.  If the optional fractal
  dimension D_frac is specified, it will be used instead of the *-p*
  value to construct the aggregates, causing the effective porosity to
  vary with size.
...
- MMF :: To apply MMF, =optool= needs the number of monomers $N$, the
  ...
  Porosity is ignored for this consideration, both monomer and grain
  volume are assumed to be compact. If the user has specified a
  fractal dimension as the second numerical value of the =-mmf=
  option, it is used for all particle sizes.  If no fractal dimension
  has been specified, the porosity $p$ is used instead to determine a
  filling factor $f=1-p$ for the aggregate. $D_f$ is then chosen
  (different for every size) so achieve that average volume filling
  fraction. That process also sets the fractal prefactor $k_f$ in a
  way that the asymptotic density of small aggregates is the monomer
  material density.  The following table summarises the relevant
  equations.
