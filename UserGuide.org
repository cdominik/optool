...
installed, or if your system lacks multithreading support, these
switches can modify the compilation:

: make ifort=true         # compile using the Intel compiler
: make fitsio=true        # add support for writing fits files with cfitsio
: make multi=false        # do not multithread (default is to do it for -d)
