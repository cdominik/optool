...
- a useful collection of compiled-in materials for standard applications
  in astronomy
- an easy way to use externat refractive index data for more
  specilized applications
...
produce input files for RADMC-3D, one for each grain size, with full
scattering matrix.
...
+ =-s [NANG]= ::
  ...
  optionally specify the number of equally-spaced steps to cover the
  range of angles between 0 and 180 degrees.  The default for =NANG=
  is 180.

+ =-chop NDEG= ::

  Cut out the first =NDEG= degrees of forward scattering and
  compensate by a reduction in the scattering cross section.  See
  [[#forward-scattering-peak][the Appendix]] for more details.
...
+ =-radmc [LABEL]= ::
  ...
  RADMC-3D uses a different normalization for the [[#normalization][scattering matrix]],
  so the output has to be adapted to serve as input for it.  In
  addition, the extension of the files will be changed to =.inp=, and
  if you specify =LABEL=, it will be used in the file name(s).
...
- dustkappa.fits ::
  ...
  N_ang-1), for each wavelength value. The stored are F_11, F_12,
  F_22, F_33, F_34, and F_44.
...
Note that the output in the FITS files contains 180 angles for the
scattering matrix, from 0.5-179.5 degrees, because this is what
Michiel Min's code produces and what MCMax uses.  For the ASCII files,
we interpolate and extend to a grid of 181 grid points from 0 to 180
degrees, because RADMC-3D requires it.
...
1. Give your =lnk= file a name exactly like
   ...
   the material and =Dorschner1995= is the reference.
