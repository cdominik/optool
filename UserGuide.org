...
+ =-dhs [FMAX]= ::
  ...
  theory), i.e. perfectly regular grains. For backward compatibility,
  *-fmax* can be used instead of *-dhs*.

+ =-mie= ::

  Do a standard Mie calculation.  This is short for *-dhs 0*.
...
- MMF :: To construct fluffy/fractal aggregates, =optool= needs the
  ...
  simply $N=a^3/a_0^3$, where $a_0$ is the radius of the monomer.  If
  the user has specified a fractal dimension $D_f$ by setting the
  second numerical value of the =-mmf= option to a number larger than
  1, it is used for all particle sizes, resulting in decreasing volume
  filling factors as a function of aggregate size (unless $D_f=3$).
  If that argument is less that 1, it is interpreted as the volume
  filling factor $f$, which will be kept for all aggregate sizes,
  adjusting the fractal dimension as needed.  The fractal prefactor
  $k_f$ in chosen so that the asymptotic density of small aggregates
  is the monomer material density.  The following table summarises the
  relevant equations.
