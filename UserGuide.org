...
+ =-a FILE= ::
  ...
  in [[#sizedist][appendix B]].  To get an example file =sizedist.dat=, run =optool=
  with the option *-wsd*.
...
+ =-l FILE= ::
  ...
  =lambda.dat=, run =optool= with the option *-wlam*. An [[#lnk-files][=lnk=]] file
...
* COMMENT Input files
=optool= has been designed to work without input files, pretty much
everything can be done right from the command line.  However, for
non-regular wavelength grids or special grain size distribution, you
can use an input file non-the-less.  This section describes the
structure of these files.

All the input files can start with a comment section of lines that
start with =*=, =!=, or =#=.
...
| normal[fn:7] distribution, triggered by $\sigma<0$ | \quad $n(a)\propto \exp\left[-\frac{1}{2}\left(\frac{a-a_{\rm m}}{\sigma}\right)^2\right]$       |
