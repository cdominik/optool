...
#+LaTeX: \centerline{by Carsten Dominik, Michiel Min, and Ryo Tazaki; June 2023, version 1.9.12}
...
[fn:12] The default limit is x_lim=10^{8}.  =optool= does the alternative
...
5. =optool= became slower in version 1.9.9 when computing the
   /short-wavelength/ properties of /very large grains/ because we
   changed the size parameter limit for falling back to a Mie
   calculation from $10^4$ to $10^8$, to make sure users get optimal
   results out of the box. You may use *-xlim 1e4* to get the old
   speed back, for applications where the short wavelengths opacity is
   dominated by small grains instead of large ones.
