...
execution path.  If you prefer to use the [[https://software.intel.com/content/www/us/en/develop/tools/compilers/fortran-compilers.html][Intel fortran compiler]], if you
have the [[https://heasarc.gsfc.nasa.gov/fitsio/][=cfitsio=]] library installed and would like to write the
opacities into FITS files instead of ASCII, or if you need more speed
while using the *-d* switch, use one or more of these parameters
during compilation:

: make ifort=true         # use the Intel fortran compiler iFort
: make fitsio=true        # add support for writing FITS files
: make multi=true         # enable multithread support for -d switch
...
- I started working on =optool= as a little side project in 2019,
  during a sabbatical in Heidelberg and Nice, made possible by
  finantial support from the DFG and the Observatoire de la Côte
  d'Azur in Nice. After a long hibernation, some free time during the
  corona virus lockdown in 2020 enabled me to finish the project.
