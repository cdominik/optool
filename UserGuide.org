...
- Standard Mie solver for spherical grains
- DHS (Distribution of Hollow Spheres) for shape effects and low-porosity aggregates
- MMF (Modified Mean Field) method to model high-porosity and fractal
  aggregates
...
+ =-dhs [FMAX]= ::
  ...
  with volume fractions between 0 and f_max (default 0.8) are
  averaged.  f_max=0 means to use solid spheres (Mie theory),
  i.e. perfectly regular grains.

+ =-fmax FMAX= :: Alternative way to set f_max, since DHS is the
  default method. 
...
+ =-mmf [AMONO]= ::
  ...
  appropriately. a_mono is the monomer radius, with default 1\mu{}m.
  Monomers will have the composition given by the *-c* and *-m*
  switches. For details, see the [[#internals][appendix]].

+ =-mie= ::

  Do a standard Mie calculation.  This is equivalent to *-fmax 0* or
  *-dhs 0*.
