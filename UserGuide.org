...
works simply by specifying the directory as the second argument. In a
new session, because of the =cache=True= flag, if the directories still
exist, the same commands just read the computed opacities back in -
this is much faster than recomputing them every time.  Be careful,
however, when you change the command. Then you need to remove the
directories to ensure new opacities will be computed.


#+begin_src python
import optool
import numpy as np
sil  = optool.particle('optool -d -a 0.001 100 0 100 ol-mg50','sil',cache=True)
carb = optool.particle('optool -d -a 0.001 3.0 0 50  c','carb',cache=True)
#+end_src
...
#+begin_src python
p_ism = 0.01*ptot                 # dilute the opacity
...
- If you get oscillations in the opacities at long wavelengths, the
  grain size resolution is not sufficient.  Use more grain sizes (see
  the *-a*, *-na* and *-d* switches).
