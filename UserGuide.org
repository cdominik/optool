...
- dustkapscatmat.dat ::
  ...
  full scattering matrix only for some wavelengths (e.g. the ones that
  will be used for image generation). Use =-sparse LAM= or =-sparse
  LAM1 LAM2= to define a wavelength (interval) for the matrix to be
  stored. Multiple =-sparse= switches are allowed.
