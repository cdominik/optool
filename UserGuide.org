...
+ =[-c] KEY-or-FILE [MFRAC]= ::
  ...
  [[#builtin-materials][key for a builtin material]], or the path to an =lnk= file. =MFRAC= is
  the /mass/ fraction (default 1.0) of the material. You can give up
  to 10 materials to build up the grain.  Mass fractions do not have
...
opacity files for use in a radiative transfer tool. Here is how to
use the module:
...
The =plot()= method applied to a particle will produce the following
plots:
...
with factors and adding them), for example to produce opacites for a
