...
- DHS :: In order to simulate irregularities in grains (irregular
  ...
  of 10^4, DHS runs internally with a fixed $x_{\rm lim\_dhs}=10^4$, a
  very reasonable simplification. In rare cases, =DMiLay= might not
  converge with large $x$. =optool= then falls back to a Mie
  computation[fn:9], using the routine =MeerhoffMie= (de Rooij+ 1984), also
  with a size parameter limit.\\
  In a situation where the imaginary part of the refractive index is
  extremely small, numerical inaccuracies in the =DMiLay= routine may
  lead to an unphysical result with $q_{\rm sca}>q_{\rm ext}$,
  implying a negative $q_{\rm abs}$. =optool= therefore enforces
...
[fn:9] You can force the switch to a Mie sphere to happen at smaller
$x$ by using something like =-xlim 1e3=.  This will speed up the
computations for large grains significantly, but introduce some small
errors. Do this only if the short wavelength opacities are dominated
by smaller grains in your application.
