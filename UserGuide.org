...
| =amean=, =asig=    | =float[np]=           | Centroid and width of (log-)normal distrbution  |
...
- Size distribution :: =optool= implements powerlaw, log-normal, and
  normal size distributions.  Each of these will be
  subject to a minimum and a maximum grain size. The grain size grid
  is logarithmic, so $da\propto a$. The logarithmic bins are then
  filled according to:
  ...
  | normal, triggered by $\sigma<0$     | \quad $n(a)\propto \exp\left[-\frac{1}{2}\left(\frac{a-a_m}{\sigma}\right)^2\right]$       |
