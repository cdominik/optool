...
+ =-dhs [FMAX]=  \hfill{}(-mie is short for -dhs 0) ::
...
+ =-xlim XLIM= ::
  Set size parameter (default 10^4) for [[#internals][speed-related switch from DHS
  to Mie]].
...
- You can cache results for quick reloading in a new python session.
  When running the following command twice, optool will be called only
  the first time.
  #+begin_src python
...
- DHS :: In order to simulate irregularities in grains (irregular
  ...
  of 10^4, =optool= switches from DHS to Mie for speed. Use =-xlim
  1e8= to trade computing time for more accurate DHS results at large
  $x$. Even then, DHS runs internally with a fixed $x=10^4$ to ensure
  stability. In rare cases, =DMiLay= might not converge and return an
  error. =optool= then falls back to a Mie computation, using the
  routine =MeerhoffMie= (de Rooij+ 1984), also with a size parameter
  limit to ensure stability.  In a situation where the imaginary part
  of the refractive index is extremely small, numerical inaccuracies
  may lead to an unphysical result with $q_{\rm sca}>q_{\rm ext}$,
  implying a small negative $q_{\rm abs}$. =optool= therefore enforces
  $q_{\rm abs}>q_{\rm ext}/10^4$.
...
- If you are interested the /short-wavelength/ properties of a /very
  large grain/, you may want to use *-xlim 1e8* to make sure you get
  the best results by avoiding the [[#internals][speed-related switch from DHS to
  Mie]] at $x=10^4$. The opacity differences are typically small, but
  significant changes may be seen in some scattering matrix elements.
  A large =xlim= is currently not the default because there is a
  significant cost in computational time, while the deviations are
  normally fully masked by dominating small grain opacities.
