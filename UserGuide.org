...
in the background[fn:2] and puts all computed quantities as =numpy=
...
The argument to =optool.particle()= must be a valid shell command[fn:3] to
...
|--------------------+-----------------------+-------------------------------------------------|
<<<<<<< HEAD
| =checknorm()=      | =method=              | Check normalization of scattering matrix        |
| =setnorm()=        | =method=              | Change the scattering matrix normalization      |
=======
...
[fn:2] The module runs the command as a subprocess, with output to a
temporary subdirectory.

[fn:3] As a string, or as a list like =['/path/to
