...
[fn:2] This can be used to easily specify a refractive index =m=n+ik=
and a material density =rho= in g/cm^3 for a calculation at a single
wavelengths, for example when trying to fit refractive indices.
