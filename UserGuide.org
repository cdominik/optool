...
+ =-chop NDEG= ::
  ...
  Cut out the first =NDEG= degrees of the [[#forward-scattering-peak][forward scattering peak]] and
...
intregal gives the correct result.

However, if you are using =optool= to produce scattering cross
sections of RADMC-3D, the conventions are different.  RADMC-3D
requires the values of the scattering matrix on the cell /boudaries/,
so at 0\deg, 1\deg etc to 180\deg, for a total of 181 values.  So for
for the input files for RADMC-3D, we interpolate and extend the
computed values to the cell boundaries.  In strong forward-scattering
...
handling this renormalization fully automactically.
