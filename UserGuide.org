...
: optool pyr  -a 10 30  -mmf 0.3  -p 0.95
...
+ =-mmf [A0 [DFRAC]]= ::
  ...
  opacities of highly porous or fractal aggregates.  *-c* and *-m*
  switches determine the composition of monomers.  =A0= is the
  monomer radius (default 0.1\mu{}m).  Particles will be aggregates
  with a /compact size/ given by the *-a* switch, giving rise to
  $N=a^3/a_0^3$ monomers. The /porosity/ (*-p*) specifies
  that the monomers should be spread out to reach a volume filling
  factor $f=1-p$.  Alternatively, =DFRAC= can specify the fractal
  dimension.  *-p* will then be ignored, and the effective porosity
  will vary with size.
...
- MMF :: To construct fluffy/fractal aggregates, =optool= needs the
  ...
  simply $N=a^3/a_0^3$, where $a_0$ is the radius of the monomer.
  ...
  |       | =-mmf A0 DF=     | =-mmf A0 -p p=             |
  |-------+------------------+----------------------------|
  | /     | <                | <                          |
  | $N$   | $=a^3/a_0^3$     | $=a^3/a_0^3$               |
  | $D_f$ | given by user    | $=3\ln N\,/\,\ln(N/(1-p))$ |
