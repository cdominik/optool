...
:END:
You can download and compile =optool= with these simple steps, using
the freely available GNU FORTRAN compiler [[https://gcc.gnu.org/wiki/GFortran][=gfortran=]].
...
cores for speed (recommended!), or to be able to write FITS
files[fn:1], use one or more of the following parameters during
compilation:
...
You can also find binaries for Mac and Linux at [[https://staff.fnwi.uva.nl/c.dominik/optool][my homepage]]. For shell
command line completion support, check the file =optool-complete=.
...
+ =-a AMIN [AMAX [APOW [NA]]]= ::
  ...
  *-apow*, *-na*. The defaults are 0.05 \mu{}m, 3000 \mu{}m, 3.5, and
  ...
  n_a=1 are implied.\\
  =>= If a_max is negative, the range will be $a_{\rm min}\pm a_{\rm max}$,
  and a_pow defaults to 0.
