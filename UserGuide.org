...
even simpler generic key.  Please check the files in the =lnk_data=
directory for additional information about these materials.  Most of
the refractive index datasets have a wide wavelength coverage - the
few exceptions are highlighted by bold-face numbers.  If a material is
being used outside of the measured region, =optool= will still
function, using extrapolated optical properties that will be quite
reasonable in many cases. Of course specific features in these
unmeasured regions will be missing from the the opacities.
...
- If the =computemean= method has been called first, the mean
  ...
  plot.  The mean opacities are per unit of grain mass, so the
  dust-to-gas mass ratio would still have to be applied to obtain
  opacities for a gas-dust mixture.

Other functionality in the python model includes reading and writing
=lnk= files and constructing opacities for a mix of different grain
species.  For more information, please read the documentation inside
the file =optool.py=.
...
- [[https://www.stsci.edu/~pontoppi/][Klaus van Pontoppidan]] for unpublished refractive index data of the
  c2d ice mix. 
