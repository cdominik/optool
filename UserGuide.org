...
example, =pyr= is an alias for =pyr-mg70=). Most of the refractive
index datasets have a wide wavelength coverage - the few exceptions
are highlighted by bold-face numbers.  If a material is being used
outside of the measured region, =optool= will still function, using
extrapolated optical properties.

Even the limited number of materials we have selected to include with
=optool= can certainly be daunting. To get started with some kind of
...
- The remaining lines should form three columns of data:
  \lambda[\mu{}m], and the real and imaginary parts of the refractive
  index, $n$ and $k$.
...
the wavelength range from 0.05 to 300 \mu{}m.
