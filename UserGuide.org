...
+ =-mmf [AMONO]= ::
  ...
  then be aggregates with the characteristic radius given by the *-a*
  switch.  The volume filling factor $f=1-p$ is specified through the
  /porosity/ (*-p*), and a fractal dimension will be chosen
  appropriately. a_mono is the monomer radius, with default 0.1\mu{}m.
  Monomers will have the composition given by the *-c* and *-m*
  switches.
...
The details and intricacies light scattering by small particles fill
books and feed a vast literature. =optool= implements robust
approximate methods that are fast, easy to use, and that can be used
for a wide variety of applications.  Here is a brief instruction on
when to use which =optool= method:
...
2. DHS takes into account geometric irregularities, so it makes the
   shape of solid state fetures more realistic (REF) and also produces
   realistic opacity enhancement conneced to irregularities, in
   particular enhancements in the Raghleigh limit (wavelength larger
...
3. In high-porosity aggregates, the properties of the monomers become
   ...
   aggregates, as well as asymmetry parameters. The scattering matrix
   of MMF is reliable if the phase shift is.....
...
- MMF ::  FIXME: Document here the details of MMF 
