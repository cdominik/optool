...
+ =-print [VAR]= ::
  ...
  \kappa_abs, \kappa_sca, \kappa_ext, and g, with a header. When =VAR=
  is any of =kabs=, =ksca=, =kext=, or =g=, write only that
  variable. You can use this to extract a single value, for example
  the 850\mu{}m extinction opacity of grains between 1 and 3mm:
...
- *sr^-1* or *cm^2 g^-1 sr^-1* for the scattering matrix elements,
  see Appendix [[#normalization][B.1]].
...
[fn:3] See [[#crystalline_mixing][appendix C.2]] about the treatment of crystalline materials.
...
- optool\underline{ }sd.dat & optool\underline{ }lam.dat ::
  ...
  When using the =-w= switch, it will write the grain size grid and
  the wavelength grid to two files. The files serve as example files
  for what the structure of files need to be to be read in with =-a
  file.dat= and =-l file.dat= for user-provided size and wavelength
  grids.
...
- /Particle arithmetic/: multiplying =optool.particle= objects with
  ...
  module and [[#crystalline_mixing][appendix C.1]] for examples.
...
3. If you do not remember how to reproduce a specific run, just check
   ...
   to produce the file.
4. Make sure the =optool= executable is on your binary search path.
   Or run it by giving the full path, like =./optool=.
