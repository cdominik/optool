...
+ =[-c] MATERIAL [MFRAC]= ::
  ...
  colon-separated numbers =n:k:rho=[fn:2]. =MFRAC= is
...
[fn:2] =n:k:rho= specifies a refractive index =m=n+ik= and a material
...
+ =-w= ::
  ...
  size distribution and the wavelengths grid, respectively. Also,
  write =blended.lnk= with the result of mixing refractive index
...
| ens        | pyr-c-mg96  | Mg_{0.96}Fe_{0.04}SiO3  | cryst[fn:3] |       2.8 |       *2.0* |        *99* | [[https://ui.adsabs.harvard.edu/abs/1998A%26A...339..904J][Jäger+98]]      |              | [[file:lnk_data/pyr-c-mg96-Jäger1998.lnk][pyr-c-mg96-Jäger1998.lnk]]    |
|------------+-------------+-------------------------+-------------+-----------+-------------+-------------+---------------+--------------+-----------------------------|
...
| for        | ol-c-mg100  | Mg_{2}SiO_4             | cryst[fn:3] |      3.27 |       *5.0* |         200 | [[https://ui.adsabs.harvard.edu/abs/2006MNRAS.370.1599S][Suto+06]]       | switch out?  | [[file:lnk_data/ol-c-mg100-Suto2006.lnk][ol-c-mg100-Suto2006.lnk]]     |
|            | ol-c-mg95   | Mg_{1.9}Fe_{0.1}SiO_4   | cryst[fn:3] |      3.33 |       *2.0* |        8190 | [[https://ui.adsabs.harvard.edu/abs/2001A%26A...378..228F][Fabian+01]]     | \rho ?       | [[file:lnk_data/ol-c-mg95-Fabian2001.lnk][ol-c-mg95-Fabian2001.lnk]]    |
| fay        | ol-c-mg00   | Fe_{2}SiO_4             | cryst[fn:3] |      4.39 |       *3.0* |         250 | [[https://ui.adsabs.harvard.edu/abs/2001A%26A...378..228F][Fabian+01]]     |              | [[file:lnk_data/ol-c-mg00-Fabian2001.lnk][ol-c-mg00-Fabian2001.lnk]]    |
|------------+-------------+-------------------------+-------------+-----------+-------------+-------------+---------------+--------------+-----------------------------|
...
| gra        | c-gra       | C graphite              | cryst[fn:3] |     2.16? |       0.001 |        1000 | [[https://ui.adsabs.harvard.edu/abs/2003ApJ...598.1026D][Draine+03]]     |              | [[file:lnk_data/c-gra-Draine2003.lnk][c-gra-Draine2003.lnk]]        |
...
[fn:3] See [[#crystalline_mixing][appendix D.2]] about the treatment of crystalline materials.
...
[fn:4]This file structure is also compatible with what is
needed to set the wavelength grid with *-l FILE*.

# ** Ad-hoc materials
# :PROPERTIES:
# :CUSTOM_ID: ad-hoc-material
# :END:
# For a omputation at the single wavelength, the refractive index and
# material density of a material can be specified on the command line.
...
-s=. This specifies the refractive index of $m=1.57+0.56i$ for a
material with a density of 2.08g/cm^3, and the computation of the
...
- dustkapscatmat.dat ::
  ...
  which will modify[fn:5] the output to make sure it can be used as an
  ...
  LAM= or =-sp LAM1 LAM2= to define a wavelength (interval)[fn:6] for
...
- blended.lnk ::
  ...
  =optool= mixes the optical properties of the various materials
  specified to be part of the grains into a single set of optical
  properties that are then used in the scattering computations. When
  the =-b= or =-w= switch is present, =optool= will write the result
  of the mixing process into the file =blended.lnk=.

- optool\underline{ }sd.dat & optool\underline{ }lam.dat ::

  When running =optool= with the =-w= switch, it will write the grain
  size grid and the wavelength grid to two files.  You can use this to
  verify that the grids set up are what you expected.  The files are
  also example files for what the structure of files need to be to be
  read in with =-a file.dat= and =-l file.dat= for user-provided size
  and wavelength grids.

[fn:5] This includes a change of the angular grid and a change in the
normalization of the scattering matrix. The format number will be 1
(or 101 for a sparse file).

[fn:6] The file will always have the matrix for at least two adjacent
...
in the background[fn:7] and puts all computed quantities as =numpy=
...
command[fn:8] to run =optool=, if necessary with the full path to the
...
[fn:7] The module runs the command as a subprocess, with output to a
temporary subdirectory.

[fn:8] As a string, or as a list like =['/path/to
...
the following units
- *microns* for grain sizes and wavelengths.[fn:9]
...
[fn:9]When giving a grain size or a wavelength on the command line,
...
| normal distribution[fn:10], triggered by $\sigma<0$       | \quad $n(a)\propto \exp\left[-\frac{1}{2}\left(\frac{a-a_{\rm m}}{\sigma}\right)^2\right]$       |
...
[fn:10]   A normal distribution is not sampled symmetrically on a
...
- DHS :: In order to simulate irregularities in grains (irregular
  ...
  DHS to Mie[fn:11]. In rare cases, =DMiLay= might not converge and
...
- MMF :: To construct fluffy/fractal aggregates, =optool= needs the
  ...
  the *-a* switch is interpreted as the /compact/[fn:12] size of all
...
[fn:11] The default limit is x_lim=10^{8}.  =optool= does the alternative
...
to the DHS opacities. See also point 5 under [[#trouble_shooting][Troubleshooting]].

[fn:12]still including the porosity specified with the *-p* switch
...
3. If you do not remember how to reproduce a specific run, just check
   ...
   to produce the file.
4. If after successful compilation the command =optool= does not work
   in your environment, make sure =optool= is on the binary search
   path of your shell, or run it by giving the full path to the
   executable, like =./optool=.
5. You may have noticed that =optool= became slower in version 1.9.9
   ...
   are completely dominated by smaller grains. We did increase the
   limit to make you get the best possible results out of the box.
