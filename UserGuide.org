...
into =bindir=. Or run run optool by specifying the full path, e.g.
=./optool= or =/path/to/optool=.
...
4. If you do not remember how to reproduce a specific run, just check
   ...
   to produce the file.
5. If after successful compilation the command =optool= does not work
   in your environment, make sure optool is on the binary search path
   of your shell, or run it by giving the full path to the executable,
   like =./optool=.
