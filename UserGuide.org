...
If prefer to use the [[https://software.intel.com/content/www/us/en/develop/tools/compilers/fortran-compilers.html][Intel fortran compiler]], if you have the [[https://heasarc.gsfc.nasa.gov/fitsio/][=cfitsio=]]
library installed and would like to write the opacities into FITS
files instead of ASCII, or if you need more speed while using the *-d*
switch, use one or more of these parameters during compilation:
...
+ =-d [NSUB]= ::
  ...
  The default for NSUB is 5. The computation will be done on multiple
  threads, unless multithreading was turned off during [[#compilation][compilation]].
...
** Units
The input and output of =optool= uses the following units
