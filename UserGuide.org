...
- MMF :: To construct fluffy/fractal aggregates, =optool= needs the
  ...
  the *-a* switch is interpreted as the /compact/[fn:8] size of all
  ...
  aggregate size.  If the parameter is $\le1$, it is interpreted as a
  ...
  set to zero. However, the opacities will remain applicable. You can
  request to get the result computed under the assumption of single
  scattering at wavelengths where the phase shift is too large. This
  result maybe usable for absorbing materials, but we do not have a
  clear criterion on when it will be accurate. To get this result, use
  the *-mmfss* instead of *-mmf*. The optool run will let you know
  below which wavelength the scattering matrix needs to be used with
  caution.
