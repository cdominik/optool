...
- =Python= interface module for easy post-processing and cross-tool
  use.

** Terms of use

=op tool= is distributed under the [[https://opensource.org/licenses/MIT][MIT license]] and can be used, changed
...
string like =pyr-mg70=. In each material class I have selected a
useful default, accessible with an even simpler generic key.  Please
check the files in the =lnk_data= directory for additional information
about these materials.  Most of the refractive index datasets have a
wide wavelength coverage - the few exceptions are highlighted by
bold-face numbers.  If a material is being used outside of the
measured region, =optool= will still function, using extrapolated
optical properties that will be quite reasonable in many cases. Of
course specific features in these unmeasured regions will be missing
from the the opacities.
...
| pyr      | pyr-mg70   | Mg_{0.7}Fe_{0.3}SiO_3   | amorph  |      3.01 |         0.2 |         500 | [[https://ui.adsabs.harvard.edu/abs/1995A%26A...300..503D][Dorschner+95]]  |              | [[file:lnk_data/pyr-mg70-Dorschner1995.lnk][pyr-mg70-Dorschner1995.lnk]]  |
...
|----------+------------+-------------------------+---------+-----------+-------------+-------------+---------------+--------------+-----------------------------|
| c        | c-z        | C                       | amorph? |       1.8 |        0.05 |         1e4 | [[https://ui.adsabs.harvard.edu/abs/1996MNRAS.282.1321Z][Zubko+96]]      |              | [[file:lnk_data/c-z-Zubko1996.lnk][c-z-Zubko1996.lnk]]           |
...
|----------+------------+-------------------------+---------+-----------+-------------+-------------+---------------+--------------+-----------------------------|
| h2o      | h2o-w      | Water ice               | cryst   |      0.92 |        0.04 |         2e6 | [[https://ui.adsabs.harvard.edu/abs/2008JGRD..11314220W][Warren+08]]     |              | [[file:lnk_data/h2o-w-Warren2008.lnk][h2o-w-Warren2008.lnk]]        |
...
| co       | co-a       | CO                      | amorph  |      0.81 |       *3.8* |       *5.8* | [[https://ui.adsabs.harvard.edu/abs/2006PCCP....8..279P][Palumbo+06]]    |              | [[file:lnk_data/co-a-Palumbo2006.lnk][co-a-Palumbo2006.lnk]]        |
|          | co2-a/c    | CO_2                    | am/cr   |       1.2 |       *2.5* |        *20* | [[https://ui.adsabs.harvard.edu/abs/2020ApJ...901...52G][Gerakines+20]]  |              | [[file:lnk_data/co2-a-Gerakines2020.lnk][amorph]]/[[file:lnk_data/co2-c-Gerakines2020.lnk][cryst]]                |
|          | ch4-a/c    | CH_4                    | am/cr   |      0.47 |       *2.0* |        *20* | [[https://ui.adsabs.harvard.edu/abs/2020ApJ...901...52G][Gerakines+20]]  |              | [[file:lnk_data/ch4-a-Gerakines2020.lnk][amorph]]/[[file:lnk_data/ch4-c-Gerakines2020.lnk][cryst]]                |
|          | ch3oh-a/c  | CH_{3}OH                | am/cr   | 0.78/1.02 |       *2.0* |        *24* | [[https://ui.adsabs.harvard.edu/abs/2020ApJ...901...52G][Gerakines+20]]  |              | [[file:lnk_data/ch3oh-a-Gerakines2020.lnk][amorph]]/[[file:lnk_data/ch3oh-c-Gerakines2020.lnk][cryst]]                |
...
- The next line contains two numbers, the number of wavelengths
  $n_\lambda$ and the specific density \rho of the material in g/cm^3
...
** Plotting the opacities and the scattring matrix
...
- a plot showing the opacities \kappa_abs, \kappa_sca, and \kappa_ext
  ...
  (on a linear y-scale).  Note that the blue /g/ curve does not havex
...
# Other functionality in the python module includes multiplying
# "particles" by factors and adding "particles" in order to construct
# opacities for a mix of different grain species.  There is also a class
# for reading, converting and writing =lnk= files. For more information,
# please read the documentation inside the file =optool.py=.
...
** Particle arithmetic
You can also use /particle arithmetic/ (multiplying =particle= objects
with a factor and adding them), for example to produce opacites for a
mixture of different grains or scale opacities using a dust-to-gas
ratio:

#+begin_src python
p1 = optool.particle('optool -a 0.01 0.3 pyr-mg70')      # silicate grains
p2 = optool.particle('optool -a 0.03 0.1 c-z')           # carbon grains
p  = 0.75*p1 + 0.25*p2   # add with mass fractions
p  = p * 0.01            # apply dust-to-gas ratio -> opacity per gas mass
#+end_src

** Preparing =lnk= files

The python module also contains tools to prepare files with refractive
index data for use with optool. The standard format of =lnk= files for
=optool= is described ?????.  These files can be read and the data
plotted using the =lnktable= class, like this:

#+begin_src python
import optool
d = optool.lnktable('lnk_data/co-palumbo2006.lnk')
d.plot()
#+end_src

The class can also read files that are formatted differently, in order
to create properly formatted version. Let's say you have a file
starting with 4 unimportant lines, and then data columns where n an k
are in column 1 and 2, respectively, and the wavelength is given in
units of cm^-1 in column 3, you can do the conversion in this way:
...
new.smooth(5)              # smooth the data, using median filter 5 pixel          
new.decimate(3)            # reduce file size by using ony every 3rd value
...
** Mean opacities in a system with evaporation
...
- DHS :: In order to simulate irregularities in grains (irregular
  ...
  1981).  When the size parameter $x=2 \pi a/\lambda$ exceeds
  a value of 10^4, no DHS averaging is used.  A standard Mie
...
- MMF :: To construct fluffy/fractal aggregates, =optool= needs the
  ...
  the /fractal dimension/ $D_f$.  Using a fixed fractal dimension
  means that the volume filling factor will decrease with aggregate
  size.  As an alternative, the structure parameter can be less
  than 1.  In that case, it is interpreted as a fixed /volume filling
  factor/ $f$ that applies to all aggregate sizes - with the
...
* Acknowledgments
- [[https://www.researchgate.net/profile/Charlene_Lefevre][Charléne Lefévre]] for [[https://github.com/charlenelefevre/SIGMA][SIGMA]], for inspiration about grain mantles.
...
the wavelength range from 1 to 300 \mu{}m.
