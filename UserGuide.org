...
=optool= comes with a python module =optool.py= that can run =optool=
in the background[fn:3] and put all computed quantities conveniently
into =numpy= arrays inside a python object.  This makes it very
straight forward to produce input files for a wide range of radiative
transfer tools or to inspect and further process the output. The
interface is used like this:

: import optool
: p = optool.particle('~/bin/optool pyr 0.8 -m ice 0.2 -na 24 -d')

The argument to =optool.particle()= must be a valid shell command to
run =optool=.  Depending on the presence of the *-d* switch, the
command will produce opacities either for =np=1= particle, of for
=np=na= particles. Most of the attributes (with the exception of the
global wavelenth and angular grids) will therefore be arrays with the
first dimension equal to =np=.  This is true even if only 1 particle
is computed. The object returned will have the following attributes:

#+ATTR_LATEX: :font \small\sf  :align llp{8cm}
| *Attribute*      | *Type/Shape*        | *Quantity*                                              |
|------------------+---------------------+---------------------------------------------------------|
| cmd              | string              | The full command given in the particle() call           |
|------------------+---------------------+---------------------------------------------------------|
| radmc            | bool                | Output follows RADMC conventions                        |
| scat             | bool                | Scattering matrix is available                          |
|------------------+---------------------+---------------------------------------------------------|
| nlam             | int                 | Number of wavelengths points                            |
| lam              | float[nlam]         | The wavelength grid, same for all sizes                 |
| nang             | int                 | Number of scattering angles                             |
| scatang          | float[nang]         | The angular grid, same of all sizes                     |
|------------------+---------------------+---------------------------------------------------------|
| np               | int                 | Number of particles computed, either 1 or (with -d) n_a |
|------------------+---------------------+---------------------------------------------------------|
...
| apow             | float[np]           | Power law of size distribution between amin and amax    |
...
|------------------+---------------------+---------------------------------------------------------|
| plot()           | method              | Plot the crosssections and matrix elements              |


The =optool.plot()= method will produce the follwong plots:
...
temporary subdirectory of the current working directory.  It then
