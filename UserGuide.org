...
with factors and adding them - this is implemented in a way that it
works correctly also for the scattering matrix), for example to
produce opacites for a mixture of different grains or scale opacities
using a dust-to-gas ratio:
