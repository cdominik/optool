...
#  If the APOW element looks like =AP:SIG=, the size distribution
#  will be log normal, centered around a_p: $dn/da = 
#  \exp\{-0.5[\ln(a/a_{\rm p})/\sigma]^2\}$
...
You can find additinal refractive index data [[https://github.com/cdominik/refind-for-optool][here]], in databases like
...
| =a1=, =a2=, =a3=    | =fload[np]=           | Mean <a>, $\sqrt{<a^2>}$, and $\sqrt[3]{<a^3>}$ |
...
- a =lnktable= class to read, plot and modify =lnk= files.
  #+begin_src python
...
- /particle arithmetic/: multiplying =optool.particle= objects with
  ...
  precmoputed set of opacities. The following page contains a number
  of examples.
#+LATEX: \newpage

Pre-compute the opacities of 100 olivine silicate grain sizes and of
50 carbon grain sizes, and store the opacities in directories. This
works simply by specifying the directory as the second argument.

#+begin_src python
import optool
sil  = optool.particle('optool -d -a 0.001 100 0 100 ol-mg50',dir='sil')
carb = optool.particle('optool -d -a 0.001 3.0 0 50  c',dir='carb')
#+end_src

In a new session, read the opacities computed above back in.  This
will be much faster than recomputing them every time.

#+begin_src python
import optool
import numpy as np
sil  = optool.particle(dir='sil')
carb = optool.particle(dir='carb')
#+end_src

Apply powerlaw size distributions, and limit the size of the
contributing grains.  Note that a power law f(a)\propto a^{-3.5} on a
logarithmic grid implies using a power a^{-2.5}.
...
only data in a small range, where the virbrational transitions lie.
...
* Troubleshooting
- If you get oscillations in the opacities at long wavelengths, the
  grain size resolution is not sufficient.  Use more grain sizes.
