...
- Release 1.9.4 ::
  - Improve the installation process and instructions.
  - Get help about specific command line options with, e.g.
    =optool -h mmf=.
  - Programmed shell completion of command line options and arguments
    is now supported with the =optool-complete= script.  You need to
    link it into your shell setup using the instructions in
    =optool-complete=.
